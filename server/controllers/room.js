...
// Function to handle renting a room
export const rentRoom = tryCatch(async (req, res) => {
  ...
  if (room.status === 'rented') {
    return res.status(400).json({ message: "This room is already rented" });
  }

  room.status = 'rented';
  await room.save();
  res.json({ success: true, message: "Room rented successfully", result: room });
});
